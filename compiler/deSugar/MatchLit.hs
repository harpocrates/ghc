...
warnAboutOverflowedLiterals
  :: DynFlags
  -> Either (HsOverLit GhcTc) (HsLit GhcTc) -- ^ the possibly overflowing lit
  -> DsM ()
...
matchNPlusKPats (var:vars) ty (eqn1:eqns)
  = do  { let NPlusKPat _ (L _ n1) (L _ lit1) lit2 ge minus = firstPat eqn1
        ; dflags      <- getDynFlags
        ; lit1_expr   <- dsOverLit dflags lit1
        ; lit2_expr   <- dsOverLit dflags lit2
